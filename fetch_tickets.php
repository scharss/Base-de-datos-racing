...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($type === 'email') {
        ...
        $sql = "SELECT p.ID AS order_id, oim.meta_value AS tickets
                ...
                JOIN wpmd_postmeta pm ON p.ID = pm.post_id

                JOIN wpmd_woocommerce_order_items oi ON p.ID = oi.order_id

                JOIN wpmd_woocommerce_order_itemmeta oim ON oi.order_item_id = oim.order_item_id
    ...
    if ($result->num_rows > 0) {
        ...
        echo '<div class="alert alert-warning" role="alert">No se encontraron códigos de respaldo para el ' . ($type === 'email' ? 'correo electr��nico' : 'n��mero de tel��fono') . ' proporcionado.</div>';
